...
if [ ! -f "$input" ]; then
  ...
  echo "usage: scripts/smoke-roughcut.sh [input-file]"
  echo "example: BYOM_VIDEO_PYTHON=.venv/bin/python scripts/smoke-roughcut.sh media/Untitled.mov"
  exit 0
fi
...
if ! "$python_bin" -c "import faster_whisper" >/dev/null 2>&1; then
  ...
  echo "or run with: BYOM_VIDEO_PYTHON=.venv/bin/python scripts/smoke-roughcut.sh $input"
  exit 1
fi

output="$(BYOM_VIDEO_PYTHON="$python_bin" ./byom-video run "$input" --with-transcript --with-chunks --with-highlights --with-roughcut --transcript-model-size tiny)"
echo "$output"
