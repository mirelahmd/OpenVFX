...
echo "==> Agent result before conversion"
"$BINARY" agent-result "$PLAN_ID" >/dev/null && pass "agent-result works before conversion" || fail "agent-result should work before conversion"
"$BINARY" agent-run "$PLAN_ID" --dry-run >/dev/null && pass "agent-run dry-run works" || fail "agent-run dry-run should work"

echo "==> Bridge approve and convert"
"$BINARY" agent-run "$PLAN_ID" --yes --convert --approve-jobs >/dev/null && pass "agent-run converted approved jobs" || fail "agent-run should convert"
"$BINARY" agent-plan-jobs "$PLAN_ID" >/dev/null && pass "agent-plan-jobs works" || fail "agent-plan-jobs should work"
"$BINARY" agent-result "$PLAN_ID" --write-artifact >/dev/null && pass "agent-result writes artifact" || fail "agent-result should write artifact"
...
"$BINARY" queue >/dev/null && pass "queue works after conversion" || fail "queue should work"

[[ -f ".byom-video/agent_plans/$PLAN_ID/linked_jobs.json" ]] && pass "linked_jobs.json exists" || fail "linked_jobs.json should exist"
[[ -f ".byom-video/agent_plans/$PLAN_ID/agent_result.md" ]] && pass "agent_result.md exists" || fail "agent_result.md should exist"
[[ -f ".byom-video/agent_plans/$PLAN_ID/agent_run_summary.json" ]] && pass "agent_run_summary.json exists" || fail "agent_run_summary.json should exist"
